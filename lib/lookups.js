...
async function fetchPromoPriceByProductId(client, promoPricebookId) {
  const soql =
    `SELECT Product2Id, UnitPrice FROM PricebookEntry ` +
    `WHERE Pricebook2Id = '${escapeSoqlValue(promoPricebookId)}' AND IsActive = true`;
  const rows = await client.queryAll(soql);
  const map = new Map();
  for (const r of rows) map.set(r.Product2Id, r.UnitPrice);
  return map;
...
async function fetchProducts(client, ids, updatedAfter) {
  // The 15 attribute fields are pulled from Product2 here alongside the 10
  // "direct" Product2 fields. Per data-team guidance, Product2 is the
  // authoritative source today; ProductAttribute is populated downstream of
  // Product2 and is not trusted yet. To revert to ProductAttribute sourcing,
  // drop ATTRIBUTE_FIELDS_FROM_PRODUCT2 from the SELECT here and re-enable
  // the fetchVariantAttributes call in export-products.js.
  const customFields = [
    ...Object.values(PRODUCT2_FIELDS),
    ...Object.values(ATTRIBUTE_FIELDS_FROM_PRODUCT2),
  ].join(', ');
  return queryChunked(client, (inList) => {
    ...
    //
    // Active_on_Parts_Store__c = true keeps products off the Parts Store
    // catalog unless they are explicitly flagged active. Per-record filter:
    // each Product2 honors its own flag. `= true` excludes both false and
    // null, so a product with no value set is treated as inactive.
    let soql =
      `SELECT Id, Name, ProductCode, StockKeepingUnit, Description, ` +
      `${PART_SHORT_DESC_FIELD}, ProductClass, Shoppable__c, ${customFields} ` +
      `FROM Product2 WHERE Id IN (${inList}) AND IsActive = true ` +
      `AND Active_on_Parts_Store__c = true ` +
...
// NOTE: currently unused. Kept in place so it can be re-enabled quickly if
// the data team decides ProductAttribute is the correct source for the 15
// attribute fields. To re-enable, uncomment the call site in
// export-products.js and remove ATTRIBUTE_FIELDS_FROM_PRODUCT2 from the
// SELECT in fetchProducts above.
