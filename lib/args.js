...
Options:
  ...
  --promo-pricebook-id <id>
                         ...
                         product.
  --custom-domain        This export targets a custom domain. Build image
                         URLs without the /cdn-cgi/image/ transform segment.
                         Cloudflare rejects the transform on custom domains
                         (cf-resized: err=9401), so the segment only costs a
                         wasted round trip there. Leave it off for
                         *.my.site.com, where the transform works.
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case '--custom-domain': out.customDomain = true; break;
      case '-h':
      case '--help':
        console.log(USAGE);
        process.exit(0);
