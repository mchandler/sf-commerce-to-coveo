...
// storefront path. This is the Saltbox form every *.my.site.com storefront has
// always used, and the transform genuinely works there (width=100 shrank a
// sample image from 10270 to 2589 bytes).
//
// A custom domain gets no /cdn-cgi/image/ segment at all. Cloudflare rejects
// the transform outright on those hosts — `cf-resized: err=9401`
// ("Transformation origin is not in allowed origins list"), which surfaces as a
// 403 for the bare format=auto. So the segment buys nothing there except a
// wasted round trip; the plain origin URL serves the image directly.
const DEFAULT_TRANSFORM = 'format=auto';

function buildImageUrl(siteUrl, contentKey, customDomain = false) {
  ...
  const transformSegment = customDomain ? '' : `cdn-cgi/image/${DEFAULT_TRANSFORM}/`;
  return `${origin}/${transformSegment}${storefrontSegment}sfsites/c/cms/delivery/media/${contentKey}${IMAGE_URL_SUFFIX}`;
}
